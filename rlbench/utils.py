...
def get_stored_demo_front(data_path, index):
    episode_path = os.path.join(data_path, EPISODE_FOLDER % index)
    ...
    num_steps = len(obs)
    for i in range(num_steps):
        front_rgb_path = os.path.join(episode_path, '%s_%s' % (CAMERA_FRONT, IMAGE_RGB), IMAGE_FORMAT % i)
        front_rgb = Image.open(front_rgb_path)
        obs[i].front_rgb = np.array(
            front_rgb)
        assert obs[i].front_rgb.shape[1] == 128, front_rgb.shape
        obs[i].misc['front_rgb_path'] = front_rgb_path

        depth_path = os.path.join(episode_path, '%s_%s' % (CAMERA_FRONT, IMAGE_DEPTH), IMAGE_FORMAT % i)
        depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
        assert depth.shape[1] == 128, depth.shape
        front_depth = image_to_float_array(
                depth,
            DEPTH_SCALE)
        obs[i].front_depth = front_depth
        obs[i].misc['front_depth_path'] = depth_path
